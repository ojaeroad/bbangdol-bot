"""회원 운영용 성과 분석 신호 저장 모듈.

기존 텔레그램 전송 및 자동매매와 독립적으로 동작한다.
DB 오류는 기존 알람 흐름으로 전파하지 않는다.
"""
...
WHERE signal_no IS NULL OR signal_no = '';

CREATE UNIQUE INDEX IF NOT EXISTS uq_performance_signals_signal_no
...
def ensure_schema() -> None:
    ...
    with _SCHEMA_LOCK:
        ...
        with _connect() as conn:
            conn.execute(_CREATE_AND_MIGRATE_SQL)
        _SCHEMA_READY = True
...
def save_signal(payload: dict[str, Any]) -> bool:
    ...
    with _connect() as conn:
        ...
        if inserted:
            ...
            signal_no = f"PF-{signal_id:010d}"
            conn.execute(
                "UPDATE performance_signals SET signal_no=%s WHERE id=%s",
                (signal_no, signal_id),
            )
            log.info(
                "Performance signal saved id=%s no=%s route=%s symbol=%s tf=%s price=%s",
                signal_id, signal_no, route, symbol, timeframe, price,
            )
            return True
...
def latest_signals(limit: int = 20) -> list[dict[str, Any]]:
    ...
    sql = """
        SELECT
            signal_no, strategy, route, exchange, symbol, side,
            signal_type, timeframe, timeframe_minutes,
